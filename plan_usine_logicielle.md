...
> | 📁 13. Output daté par run (isolation des artefacts) | ❌ à faire *(constat : Coder écrit dans cwd = racine projet → bubble_sort/ + landing_page/ pollués, écrasement des runs, pas de traçabilité)* |
>
...
- [ ] **Isolation datée des sorties** : faire écrire le Coder/Tester dans un répertoire dédié par run (ex: `runs/{date}_{slug}/`). **Option A (recommandée) — `chdir` global** : changer le cwd vers le output_dir daté avant l'exécution des nœuds Coder/Tester dans `run_coding_workflow`. Le + propre, les chemins relatifs des `target_files` continuent de marcher naturellement. **Point d'attention critique** : `kg_path` doit être résolu en **absolu avant le chdir** (sinon la DB DuckDB change de place à chaque run → perte de la mémoire entre runs / reprise cassée). **Option B — préfixer les target_files** : muter `ArchitectTask.target_files` (`landing_page/index.html` → `runs/.../landing_page/index.html`). Plus invasif, risque de casser le dispatch techno du Tester (détection par extension). À évaluer.
- [ ] *(complément)* **Slug du nom de run** : dériver un slug lisible depuis le contenu de la tâche (ex: `bubble_sort_visualizer`) plutôt que seulement le `run_id` (hash). Éviter les caractères interdits sur Windows (`:`, `?`, `*`).
- [ ] *(complément)* **Nettoyage optionnel** : config `OUTPUT_RETENTION` (nb de runs à garder, ex: 10) pour éviter que le dossier `runs/` ne grossisse indéfiniment. À ajouter dans `.gitignore` (`runs/` déjà implicitement non versionné mais à expliciter).
- [ ] *(référence)* **Point d'insertion** : `run_coding_workflow` (`workflows.py:206`), après calcul du `run_id` (l.221) et AVANT la première écriture Coder. Le `run_id` (l.220, hash du contenu de tâche, PAS du cwd) reste stable — il ne faut PAS le dériver du cwd (sinon un déplacement du projet casserait la reprise).
