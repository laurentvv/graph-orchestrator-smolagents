...
async def execute_coder_node(
    ...
    """Nœud Coder : utilise des outils pour créer/éditer des fichiers.

    F-169 : moteur UNIQUE pydantic-ai-harness (décision user 2026-08-24 —
    « enlève le CodeAgent de smolagents »). L'ancienne exécution smolagents
    (CompactingCodeAgent + vision_callback + run_with_retry) est retirée :
    chaîne pydantic validée E2E (F-162 GO migration, F-166 auto-fixer porté
    aux 2 moteurs puis consolidé ici). ``fast_model`` reste dans la signature
    pour compatibilité des appelants — le moteur pydantic spawn son propre
    llama-server via model_lifecycle.
    """
