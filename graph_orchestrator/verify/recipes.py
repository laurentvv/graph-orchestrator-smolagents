"""Détection statique de recettes de vérification projet (F-100).

Port quasi 1:1 de ``references/hermes-agent/agent/verify/recipes.py`` (lui-même
porté de superagent-ai/grok-cli ``src/verify/recipes.ts``) : choix des
commandes et ordre de détection préservés — Node (package manager par lockfile
+ détection framework), Python (Django / FastAPI / Flask / générique, avec
uv/poetry/pipenv), Go, Rust, Java (Maven/Gradle), Makefile, docker-compose.

Écart consciencieux vs la référence (adaptation à l'usine) : ajout d'un
détecteur **static-web** en DERNIER recours (après tous les détecteurs grok).
Notre cas dominant (Prompt-Vault = apps HTML+CSS+JS vanilla dans ``runs/``)
n'a ni package.json ni pyproject — la référence retournerait ``None``. Le
détecteur static-web produit une recette qui sert le dossier via
``python -m http.server`` (exécutable courant, bind 127.0.0.1 pour éviter le
popup firewall Windows), rendant « la page est servie et répond » une preuve
exécutable au lieu de ``file://`` seul.

La commande ``start`` des recettes static-web contient le placeholder
``{port}`` (substitué par le runner au port effectif) — voir ``runner.py``.
"""
...
@dataclass
class Recipe:
    """Recette de vérification exécutable pour un projet.

    Miroir du ``VerifyRecipe`` grok-cli au champ près : ``name`` = label
    humain (``appLabel``), ``kind`` = id du détecteur (``appKind``), listes
    de commandes = strings shell exécutées à la racine du projet.
    """
    ...
    def from_dict(cls, raw: Any) -> "Recipe | None":
        """Loader tolérant, miroir du ``normalizeVerifyRecipe`` grok."""
...
def detect_package_manager(root: Path) -> str | None:
    """Détection du package manager par lockfile (grok ``detectPackageManager``)."""
    candidates = [
        ("pnpm-lock.yaml", "pnpm"),
        ("bun.lock", "bun"),
        ("bun.lockb", "bun"),
...
def _infer_port_from_command(command: str | None) -> int | None:
    """Inférence du port depuis une commande de start (grok ``inferPortFromCommand``)."""
    if not command:
        return None
...
# ---------------------------------------------------------------------------
# static-web (écart consciencieux vs hermes/grok — notre cas dominant Prompt-Vault)
# ---------------------------------------------------------------------------

def _detect_static_web_recipe(root: Path) -> Recipe | None:
    """Détecteur static-web : dossier d'assets HTML sans manifeste de build.

    Dernier recours après tous les détecteurs grok (un projet Vite/Next a un
    package.json → recette node, plus précise). Sert le dossier via
    ``http.server`` sur ``{port}`` (placeholder substitué par le runner) :
    proof « la page est servie et répond » sans dépendre de ``file://``
    (les scripts ES-module et fetch sont bloqués par CORS en file://).
    """
    root = Path(root)
    if (root / "index.html").exists():
        ...
        if not htmls:
            return None
        entry = htmls[0]
    # Bind 127.0.0.1 : évite le popup Windows Firewall (0.0.0.0 le déclenche).
    start = f'"{sys.executable}" -m http.server {{port}} --bind 127.0.0.1'
    return Recipe(
        name="Static web app",
        kind="static-web",
        start=start,
        readiness_path="/" if entry == "index.html" else f"/{entry}",
        evidence=[f"Detected {entry} (no build manifest)"],
    )
...
def detect_recipe(root: Path) -> Recipe | None:
    """Détecte une recette de vérification pour le projet à ``root``.

    Ordre de détection = ``inferFallbackRecipe`` grok : package.json d'abord,
    puis Python, Go, Rust, Java, Makefile / docker-compose — PUIS static-web
    (écart consciencieux, cf. docstring module). ``None`` si rien reconnaissable.
    """
    root = Path(root)
    ...
    return (
        ...
        or _detect_static_web_recipe(root)
    )
