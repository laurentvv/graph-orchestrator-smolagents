...
For each subtask, a **Coder** node wakes up. It receives a clear order and a powerful toolkit. It writes files, navigates the terminal, uses Git, and even has eyes to check UI elements visually before pushing code. The engine is being migrated to **pydantic-ai-harness** (FileSystem tools, native `CoderOutput` structured output, skills — validated at **-82% input tokens** vs the smolagents baseline; behavioral guards loop/stall/idle/goal + transport revive and tiered context compaction as native capabilities/hooks): set `CODER_ENGINE=pydantic` to switch nodes one at a time while `smolagents` remains the default (migration plan: `docs/PLAN_MIGRATION_PYDANTIC_HARNESS.md`).
