...
from graph_orchestrator.nodes import resolve_verbosity, run_with_retry
...
async def main():
    ...
    output_dir = os.path.join(os.path.dirname(__file__), "out_test_6")
    ...
    from graph_orchestrator.sanitizer import sanitize_tools
...
"""
        ...
        print(f"[*] Démarrage de l'agent full (avec LoopGuard) pour la tâche: {task['id']}")
        try:
            guard = LoopGuard(
