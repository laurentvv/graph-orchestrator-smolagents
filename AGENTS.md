...
4. **Recherche internet en cas de blocage** (leçon session boucles isolation 2026-08-22) : quand N itérations d'isolation n'élargissent plus l'espace de solutions (garde après garde sur le même symptôme), CHERCHER SUR INTERNET l'état de l'art de la panne exacte (WebSearch : « llama.cpp force output format », « assistant prefill », « anti repetition sampler »…). Cas réel : blocage « le modèle n'exécute jamais le rituel de clôture » (10 boucles d'isolation, gardes multi-fences/r'''/micro-protocoles/rituel-seul) → 3 recherches ont livré l'arsenal SERVEUR que le prompt ne pourra jamais fournir : prefill assistant natif llama-server, sampler DRY, grammaires GBNF. Méthode : chercher AVANT de coder la énième garde ; tester le levier sur un llama-server spawné à la main (minutes) AVANT de l'intégrer ; documenter dans `docs/LLAMA_SERVER_FLAGS.md`.
5. **« Forcer un modèle à obéir » — l'arsenal** (demande user 2026-08-22, par force croissante) : un ordre en prompt = probabilité, jamais certitude ; la certitude vit au niveau SERVEUR et STRUCTURE. (a) **Prefill assistant** `CODER_PREFILL_CODE=true` : le wrapper modèle append un message assistant incomplet ` ```python ` que llama-server CONTINUE nativement → la réponse démarre physiquement dans un bloc code (tue multi-fences, prose, `<think>` interminable des distills). Pièges : contenu = chaîne simple (issue llama.cpp #14353), nuances avec reasoning models (#15714). (b) **Sampler DRY** `<PREFIX>_DRY_MULTIPLIER=0.8` (+`_DRY_ALLOWED_LENGTH=2`, `_DRY_PENALTY_LAST_N=512`) : pénalise la répétition de SÉQUENCES — cible la re-dérivation en boucle du même plan. Éviter `repeat_penalty` (écrase les probas token-level). (c) **Grammaires GBNF** (réserve nucléaire) : contrainte token-level via `response_format`, MAIS inopérante quand le thinking est actif (issue #20345) → en conflit avec les modèles à `<think>` forcé. (d) Gardes observationnelles déterministes (multi-fences, r'''→`r"""`, hints d'erreur MCP) — le filet applicatif quand (a)-(c) ne s'appliquent pas.
6. **Validation humaine** : JAMAIS modifier les règles à l'aveugle — résumé clair + solution proposée (ex. « durcir cette règle dans `nodes.py` »), attendre le feu vert.
7. **Application** : intervenir dans le code source pour durcir prompts ou skills.
