...
- **Livrable parfait en une itération (2026-08-18, run #19)** : `debug/reference_run_2026-08-18_run19_perfect_deliverable/` — 100 % conforme en une itération (~14 min, 21 steps), préservation artefacts F-120 (`plan.md`, `task.md`, `draft.md`). Leçon : le 4B suit le plan à la lettre, un prompt/draft sain vaut mieux que des corrections aval.
  - ⚠️ **INVALIDÉ le 2026-08-24 (retest E2E, run 0857, re-validation F164-6)** : même tâche `bubble-sort-multifile-v6`, itération 1 en ÉCHEC — 40 steps brûlés (`coder_max_steps`), 7 audits visuels échoués (board vide, critères 1 & 5), CSS livré avec `var(--*)` consommées mais `:root` jamais écrit (thème sombre mort). Cause candidate principale : passage **Ornith-1.0 → Ornith-1.5** (GGUF installé le matin même, premier run) — le Drafter produit un draft 3× plus court (intentions sans valeurs : hauteur `(v/100)*100%` sans hauteur parent → board vide ; variables listées sans valeurs → `:root` absent). La leçon « le 4B suit le plan à la lettre » tient : le plan est devenu creux. À revalider après durcissement Drafter/draft (ex. draft_gate sur densité prescriptive) ou retour Ornith-1.0. Positif : les gardes ont tenu (fail-closed F-109, aucune fausse approbation ; itération 2 lancée proprement).
