...
4. **llama.cpp vendé (F-123, veille hebdo programmée)** : releases quasi-quotidiennes → `uv run python scripts/update_llamacpp.py` (check seul, exit 2 si nouveauté ; `--apply` = télécharge, vérifie flags critiques, swap avec backup `.bak`). JAMAIS d'`--apply` sans validation post-swap (`debug/test_mtp_spec.py --only reasoning` + tests). Guide flags : `docs/LLAMA_SERVER_FLAGS.md`.
